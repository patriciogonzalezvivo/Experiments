...
void main (void){
    vec2 st = gl_TexCoord[0].st;
	float water = texture2DRect(waterTexture, st).r;

    if(water > 0.0){
        vec2 top = noiseXY(maxNoise,speed);
        vec2 bottom = noiseXY(minNoise,speed*0.5);
        vec2 norm = texture2DRect(normalsTexture, st).rg;

        vec2 rta = mix( mix(bottom,top,water),
                        vec2(mix(bottom.x,norm.x,top.x),
                             mix(bottom.y,norm.y,top.y)),
                        norAmount);

        gl_FragColor = vec4(rta.x, rta.y, 0.5, 1.0);
    } else {
